...
import { useRoqClient, useProductCategoryFindFirst } from 'lib/roq';
...
function ProductCategoryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useProductCategoryFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ProductCategoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.product_category.update({
        data: values as RoqTypes.product_category,
        where: {
          id,
        },
      });
