...
import { useRoqClient, useBlogFindManyWithCount } from 'lib/roq';
...
export function BlogListPage(props: BlogListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'blog_comment.count'],
  ...
  const { data, error, isLoading, mutate } = useBlogFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.blog.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'content', header: 'Content', accessorKey: 'content' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.published_at ? format(record?.published_at, 'dd-MM-yyyy') : '',
    },
    { id: 'contentimg', header: 'Contentimg', accessorKey: 'contentimg' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('blog_comment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'blog_comment',
          header: 'Blog Comment',
          accessorKey: 'blog_comment',
          cell: ({ row: { original: record } }: any) => record?._count?.blog_comment,
        }
      : null,
