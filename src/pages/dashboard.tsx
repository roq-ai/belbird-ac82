...
import { useRoqPlatformUserProfiles, useProductCategoryCount, useBlogCategoryCount, useAdminRoleCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const productCategoryStat = useProductCategoryCount();

  const blogCategoryStat = useBlogCategoryCount();

  const adminRoleStat = useAdminRoleCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Product Categories',
      result: productCategoryStat?.data,
      isLoading: productCategoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Blog Categories',
      result: blogCategoryStat?.data,
      isLoading: blogCategoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Admin Roles',
      result: adminRoleStat?.data,
      isLoading: adminRoleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
