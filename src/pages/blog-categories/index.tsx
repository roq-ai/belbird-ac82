...
import { useRoqClient, useBlogCategoryFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<BlogCategoryInterface, unknown>;
...
export function BlogCategoryListPage(props: BlogCategoryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useBlogCategoryFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.blog_category.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: BlogCategoryInterface) => {
    if (hasAccess('blog_category', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/blog-categories/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/blog-categories/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="blog_category" />
        </Flex>
