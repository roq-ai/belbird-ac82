...
import { useRoqClient, useBlogCategoryFindFirst } from 'lib/roq';
...
function BlogCategoryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBlogCategoryFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BlogCategoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.blog_category.update({
        data: values as RoqTypes.blog_category,
        where: {
          id,
        },
      });
